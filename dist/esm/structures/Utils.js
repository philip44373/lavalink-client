            ...
            const r = {
                ...
                info: {
                    ...
                    author: data.info.sourceName === 'spotify'
                        ? (data.pluginInfo?.artists?.map((x) => x.name).join(', ') || 'Unknown Artist')
                        : data.info.author,
...
async function getClosestTrack(data, player) {
    ...
    if (player.LavalinkManager.utils.isTrack(data))
        return player.LavalinkManager.utils.buildTrack(data, typeof data.requester === 'object' ? data.requester : undefined);
